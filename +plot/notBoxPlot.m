...
function [h,statsOut]=myPlotter(X,Y)
 ...
 SD=std(Y);  %Requires the stats toolbox 
 mu=mean(Y); %Requires the stats toolbox 
 if markMedian
    med = median(Y);
 end
 ...
 for k=1:length(X)
     ...
     h(k).data=plot(thisX(1:5:end)+J(1:5:end), thisY(1:5:end), 'o', 'color', C,...
