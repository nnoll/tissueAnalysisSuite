    ...
    for c = extCells
        ...
        for nc = nExtCells
            ...
            if (length(bondVerts) == 2)
                ...
                rBext = [rBext;[double(Struct.Vdat(bondVerts(1)).vertxcoord)-double(Struct.Vdat(bondVerts(2)).vertxcoord), ...
                                double(Struct.Vdat(bondVerts(1)).vertycoord)-double(Struct.Vdat(bondVerts(2)).vertycoord)]];
...
%     rCext = bsxfun(@rdivide,rCext,sqrt(sum(rCext.^2,2)));
